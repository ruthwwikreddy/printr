...
# ── Print agent ─────────────────────────────────────────
# Must be identical on the hub and on the counter PC.
PRINT_AGENT_AUTH_SECRET=a-long-random-token

# ── Landing page host (optional) ────────────────────────
# Comma-separated hosts where / shows the marketing page instead of the kiosk.
NEXT_PUBLIC_LANDING_HOSTS=printr.ruthwikreddy.live
...
On the counter PC the agent reads `BACKEND_URL`, `PRINT_AGENT_AUTH_SECRET` and optionally
`PRINTER_NAME`.

## 7. Admin password

`/admin` is protected by a single password — there are no user accounts, no Firebase Auth, no OAuth.

- Set `ADMIN_PASSWORD` in the hub's environment.
- If it is unset, the development default `printr-admin` is used and the login screen tells you so.
  **Set a real password before exposing the hub to the internet.**
- Logging in stores an httpOnly, SHA-256 session cookie for 7 days; **Log out** in the sidebar clears it.
- Every mutating admin API (`stats`, `settings` POST, `pricing`, `jobs/retry`, `jobs/cancel`,
  `cleanup`) rejects requests without that cookie. `GET /api/admin/settings` stays public because the
  kiosk needs your shop name and rates.
